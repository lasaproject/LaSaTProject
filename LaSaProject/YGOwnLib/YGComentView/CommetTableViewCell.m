...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        [_commentLabel setFont:[UIFont fontWithName:@"Helvetica-Bold" size:FONT_BIG_1]];
...
-(void)setModel:(SocketBaseModel *)model
{
    ...
    [_commentLabel addAttributedWithString:_showString range:NSMakeRange(0, [NSString stringWithFormat:@"%@%@",LABEL_BLANK,model.username].length + 1) color:COLOR_MAIN];
    _levelView.levelString = model.grade;
    _levelView.frame = CGRectMake(5, 0, 0, 0);
    _levelView.centery = FONT_SMALL_1/2;
    ...
    _nameButton.frame = CGRectMake(_levelView.x+_levelView.width, 0,model.username.length *FONT_BIG_1 + 10, FONT_BIG_1);
}
