...
#define baseViewHeight DEVICE_SCREEN_HEIGHT/3.0

@implementation YGCityPikerView
{
    void(^_handler)(NSString *province, NSString *city, NSString *district);
...
{
    YGCityPikerView *cityPikerView = [[YGCityPikerView alloc] initWithHandler:handler];
    [cityPikerView show];
    return cityPikerView;
}

- (instancetype)initWithHandler:(void (^)(NSString *province, NSString *city, NSString *district))handler
{
    self = [super init];
    if (self)
    {
...
- (void)configUI
{
    ...
    UIView *baseView = [[UIView alloc] initWithFrame:CGRectMake(0, DEVICE_SCREEN_HEIGHT, DEVICE_SCREEN_WIDTH, baseViewHeight)];
    ...
    UIView *topBaseView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, DEVICE_SCREEN_WIDTH, 40)];
    ...
    [cancelButton setTitleColor:COLOR_GRAY_LIGHT forState:UIControlStateNormal];
...
- (void)yesButtonClick
{
    ...
    NSString *districtStr = _district[districtIndex];
    ...
    if (_handler)
    {
        _handler(provinceStr, cityStr, districtStr);
    }
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    if (component == PROVINCE_COMPONENT)
    {
        ...
        myView.font = [UIFont systemFontOfSize:FONT_BIG_1];
        myView.textColor = COLOR_MAIN;
            ...
            for (int j = 0; j < cityDic.count; j++)
            {
                if ([_city[j] isEqualToString:city])
                {
                    ...
                    for (int k = 0; k < disArray.count; k++)
                    {
                        ...
                        {
                            [_picker selectRow:k inComponent:2 animated:YES];
