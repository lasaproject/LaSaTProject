...
- (void)initialize{
    ...
    subviewClassName = @"PGIndexBannerSubView";
    
    [self addSubview:self.scrollView];
    
    self.pageControl = [[UIPageControl alloc] init];
    self.pageControl.frame = CGRectMake(0, 0, 100, 20);
    CGFloat centerX = self.scrollView.width * 0.5;
    CGFloat centerY = self.scrollView.height - 20;
    self.pageControl.center = CGPointMake(centerX, centerY);
    // 2.设置圆点的颜色
    self.pageControl.currentPageIndicatorTintColor = COLOR_WHITE;
    self.pageControl.pageIndicatorTintColor = COLOR_GRAY_DEEP;
...
- (void)queueReusableCell:(PGIndexBannerSubView *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    PGIndexBannerSubView *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                PGIndexBannerSubView *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    PGIndexBannerSubView *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, PGIndexBannerSubView *cell) {
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[PGIndexBannerSubView class]]) {
...
- (PGIndexBannerSubView *)dequeueReusableCell{
    PGIndexBannerSubView *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(PGIndexBannerSubView *)cell {
