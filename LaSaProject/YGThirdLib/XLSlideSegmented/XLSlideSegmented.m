...
//item间隔
static const CGFloat ItemMargin = 15.0f;
//选中放大倍数
...
- (void)layoutSubviews {
    ...
    //设置阴影
    _shadow.backgroundColor = COLOR_MAIN;
...
- (CGFloat)itemWidthOfIndexPath:(NSIndexPath*)indexPath {
    ...
    NSDictionary *attributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:FONT_NORMAL], NSParagraphStyleAttributeName : style };
...
- (CGRect)shadowRectOfIndex:(NSInteger)index {
    return CGRectMake([_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]].frame.origin.x+10, self.bounds.size.height - 2, [_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]].frame.size.width-20, 2);
}
