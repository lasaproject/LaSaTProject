...
- (void)hidePresendView
{
    [UIView animateWithDuration:0.30 delay:2 options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.frame = CGRectMake(0, self.frame.origin.y - DEVICE_STATUS_BAR_HEIGHT, self.frame.size.width, self.frame.size.height);
        self.alpha = 0;
    } completion:^(BOOL finished) {
...
//    _skLabel.borderColor = COLOR_MAIN;
    _skLabel.textColor = COLOR_MAIN;
...
- (void)setModel:(GiftModel *)model {
    _model = model;
    [_headImageView sd_setImageWithURL:[NSURL URLWithString:model.headImageUrl] placeholderImage:DEFAULT_IMAGE_PLACEHOLDER];
    [_giftImageView sd_setImageWithURL:[NSURL URLWithString:model.giftImageUrl] placeholderImage:DEFAULT_IMAGE_PLACEHOLDER];
