...
- (void)startConnectSocketSuccess:(void (^)(NSString * host,UInt16 port))successConnectServer;
{
    ...
    if ([self SocketOpen:SERVER_SOCKET_URL port:SERVER_SOCKET_PORT] )
    {
        NSLog(@"开始连接");
...
{
    NSDictionary *sendDic = @{@"type":[YGAppTool stringValueWithInt:YGSocketJSONTypeHeartBeat],@"userid":SYS_SINGLETON.user.userId,@"liveid":_liveid};
